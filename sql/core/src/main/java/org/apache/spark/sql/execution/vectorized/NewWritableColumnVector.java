 ...
 *
 * A NewWritableColumnVector should be considered immutable once originally created. In other words,
 * it is not valid to call put APIs after reads until reset() is called.
 *
 * NewWritableColumnVector are intended to be reused.
 */
public abstract class NewWritableColumnVector extends ColumnVector {
  ...
  public void reset() {
    ...
    if (childColumns != null) {
      for (NewWritableColumnVector c: childColumns) {
        c.reset();
      }
    }
    elementsAppended = 0;
   ...
   *
   * This should only be called when this `NewWritableColumnVector` represents dictionaryIds.
   ...
   */
  protected NewWritableColumnVector dictionaryIds;
   ...
   */
  public NewWritableColumnVector getDictionaryIds() {
   ...
   */
  public NewWritableColumnVector reserveDictionaryIds(int capacity) {
    if (dictionaryIds == null) {
  ...
  public final int appendArray(int length) {
    reserve(elementsAppended + 1);
    for (NewWritableColumnVector childColumn : childColumns) {
  ...
  // `NewWritableColumnVector` puts the data of array in the first child column vector, and puts the
  ...
  // `NewWritableColumnVector` puts the key array in the first child column vector, value array in the
  ...
  public NewWritableColumnVector arrayData() {
  ...
  @Override
  public NewWritableColumnVector getChild(int ordinal) { return childColumns[ordinal]; }
   ...
   */
  protected NewWritableColumnVector[] childColumns;

  /**
   * Reserve a new column.
   */
  protected abstract NewWritableColumnVector reserveNewColumn(int capacity, DataType type);
   ...
   * type.
   */
  protected NewWritableColumnVector(int capacity, DataType type) {
    super(type);
    ...
    if (isArray()) {
      ...
      this.childColumns = new NewWritableColumnVector[1];
      this.childColumns[0] = reserveNewColumn(childCapacity, childType);
    } else if (type instanceof StructType) {
      StructType st = (StructType)type;
      this.childColumns = new NewWritableColumnVector[st.fields().length];
      for (int i = 0; i < childColumns.length; ++i) {
        this.childColumns[i] = reserveNewColumn(capacity, st.fields()[i].dataType());
      }
    } else if (type instanceof MapType) {
      MapType mapType = (MapType) type;
      this.childColumns = new NewWritableColumnVector[2];
      ...
      this.childColumns = new NewWritableColumnVector[3];
