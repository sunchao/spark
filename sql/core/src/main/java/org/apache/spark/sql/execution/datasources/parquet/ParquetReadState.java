...
class ParquetReadState {
  ...
  public ParquetTypeInfo getColumnInfo() {
  ...
  /**
   * Calculate collection offsets recursively for this column.
   */
  ...
  private void calculateCollectionOffsets() {
    ...
    for (int i = 0, rowId = 0, offset = 0; i < definitionLevels.numValues();
  ...
  private void calculateStructOffsets() {
    int maxDefinitionLevel = columnInfo.definitionLevel();
    vector.reserve(definitionLevels.numValues());
    for (int i = 0, rowId = 0; i < definitionLevels.numValues(); i++, rowId++) {
