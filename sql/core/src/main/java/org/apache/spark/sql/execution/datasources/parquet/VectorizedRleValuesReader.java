...
public final class VectorizedRleValuesReader extends ValuesReader
  ...
  /**
   * Keep reading repetition level values from the page until either the page is completely
   * drained or we've seen `totalRows` values that are equal to 0, which means they represent a
   * single element of the top-level field.
   *
   * Returns a tuple where the first element is the totalRows number of levels read, and the second
   * is the totalRows number of levels read whose value is equal to 0.
   *
   * Note that `totalRows` should be <= the totalRows number of values in the page of this reader.
   */
  public int[] readRepetitionLevels(
      int totalValues,
      int totalRows,
      WritableColumnVector c) {
    int valuesToRead = totalValues;
    int rowsToRead = totalRows + 1;

    while (rowsToRead > 0 && valuesToRead > 0) {
      if (currentCount == 0 && !readNextGroup()) {
        break;
      }
      int valuesToReadInBlock = Math.min(valuesToRead, currentCount);
      switch (mode) {
        case RLE:
          if (currentValue == 0) {
            int n = Math.min(rowsToRead, valuesToReadInBlock);
            rowsToRead -= n;
            // if we reached the beginning of `(totalRows + 1)th` row, we should move back one
            // position so that it will be read again by the next call.
            if (rowsToRead == 0) {
              n--;
            }
            c.appendInts(n, 0);
            ...
            c.appendInts(valuesToReadInBlock, currentValue);
            valuesToRead -= valuesToReadInBlock;
            currentCount -= valuesToReadInBlock;
          }
          break;
        case PACKED:
          int i = 0;
          for (; i < valuesToReadInBlock; ++i) {
            int currentValue = currentBuffer[i + currentBufferIdx];
            if (currentValue == 0) {
              rowsToRead--;
              if (rowsToRead == 0) {
                // if we reached the beginning of `(totalRows + 1)th` row, we should move back one
                // position so that it will be read again by the next call.
                //
                // for bit-packed encoding, the number of values in the current group could be
                // less than 8, in which case we'll read outside the boundary but it should be
                // zero padded always.
                break;
              }
            }
            c.appendInt(currentValue);
          }
          valuesToRead -= i;
          currentCount -= i;
          currentBufferIdx += i;
    ...
    return new int[]{ totalValues - valuesToRead, totalRows + 1 - rowsToRead };
  }
    ...
    while (left > 0) {
      ...
      switch (mode) {
        case RLE:
          ...
          definitionLevels.putInts(rowId, n, currentValue);
          break;
        case PACKED:
          for (int i = 0; i < n; ++i) {
            ...
            definitionLevels.putInt(rowId + i, currentValue);
          }
          break;
      }
      rowId += n;
  ...
  public void readIntegers(
    ...
    values.setNumValues(values.numValues() + total - left);
  }
