...
case class EnsureRequirements(
  ...
  private def ensureDistributionAndOrdering(
    ...
    if (childrenIndexes.length > 1) {
      ...
      // new shuffle for A.
      val shouldIgnoreMinPartitions = specs.forall(p =>
      ...
      val candidateSpecs = specs
          ...
          .filter(p => shouldIgnoreMinPartitions || children(p._1).outputPartitioning
              .numPartitions >= conf.defaultNumShufflePartitions)
