...
trait ParquetTypeInfo {
  ...
  def withNewType(ty: DataType): ParquetTypeInfo = this match {
    case ParquetGroupTypeInfo(_, repetitionLevel, definitionLevel, required, children) =>
      ParquetGroupTypeInfo(ty, repetitionLevel, definitionLevel, required, children)
    case ParquetPrimitiveTypeInfo(_, desc, repetitionLevel, definitionLevel, required) =>
      ParquetPrimitiveTypeInfo(ty, desc, repetitionLevel, definitionLevel, required)
  }
